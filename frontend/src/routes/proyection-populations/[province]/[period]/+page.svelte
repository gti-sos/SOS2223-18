<script>
    // @ts-nocheck
        ...
        let API = "/api/v1/proyection-populations/"+province+ "/" +period;
        if(dev)
        ...
        async function getPopulations_dato(){
            ...
            resultStatus = status;
            if(status==404){
                mensajeUsuario = `La ruta solicitada "${province}/${period}" no existe`;
            }
        }
    ...
    <h1 style="text-align: center; font-family:'Times New Roman', Times, serif; font-size: 60px;"> Cambios para datos poblacionales</h1>
    ...
    <Table striped>
        ...
        <tbody>
            <tr>
                ...
                <td><input bind:value={updatePopulationsAge}></td>
